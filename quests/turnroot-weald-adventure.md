...
This is a framework rather than a single session, so it pays by the session rather than in a lump: **~40 gold per character per session** in the campaign's first six sessions, and whatever band the table has reached after that (`rules/equipment.md`, Pacing — income doubles every six sessions; multiply by party size).
