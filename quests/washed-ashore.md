...
**Hullback** (`bestiary/hullback.md`, CTR 13 — calibrated against the full party, not a single attacker, with a deliberately capped 6-card deck rather than the 13 its stats would normally buy) — a solitary, much older predator holding the rock outcroppings inland of the tideline. A real, optional risk: avoidable by giving the rocks distance, genuinely worth the danger if the party wants what it's sitting on. Its den is where the party's first actual equipment comes from — see `items/washed-ashore-items.md`.
