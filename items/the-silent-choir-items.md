...
*Which Seat, if any, this is aligned to is open design, not a gap to close — same register as the Fog Goggles (`items/fog-basin-items.md`, `rules/items.md`). A ring that isolates one target from everyone trying to help them is a small, personal version of the same silence the room itself performs at scale — the Choir keeps the world out; the Ring keeps a person alone. Scoped to a single enemy, once per combat, and doesn't touch the target's own actions — real, but bounded, matching how Cord of the Held and the Quiet Index (`items/capital-items.md`) both stay narrow rather than open-ended.*
