...
*Which Seat, if any, this is aligned to now has a real candidate — Song and Silence (`mythology/seats.md`), noticed for the same reason Fermata itself is noticed there. Still not closed on purpose, same register as the Fog Goggles (`items/fog-basin-items.md`, `rules/items.md`) and Aege's own unresolved stage — a resonance on record, not a confirmed alignment. A ring that isolates one target from everyone trying to help them is a small, personal version of the same silence the room itself performs at scale — the Choir keeps the world out; the Ring keeps a person alone. Scoped to a single enemy, once per combat, and doesn't touch the target's own actions — real, but bounded, matching how Cord of the Held and the Quiet Index (`items/capital-items.md`) both stay narrow rather than open-ended.*
