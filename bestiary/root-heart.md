...
*Core fill completed 2026-09-06; the list had declared 11 and named 9 since it was written. **TRACE** is FOREST MEMORY's mechanic in another key — both read the party's colour habits and charge them for repeating themselves, which is what a forest that "remembers every path ever walked" should feel like from the inside. **SWAY** was a core Green card until 2026-09-08, when a rewrite turned it into the pool's third identical Evade card; rather than lose the effect, it comes here as a signature, where an immobile thing that repositions everyone but itself is the whole joke — it took BRISTLE's slot (BRISTLE has since been cut). **DIG IN** is Anchored, and the Root Heart is Immobile: Anchored ends when its holder moves, so on the one creature that can never move, it never ends. Its immobility stops reading as a limitation and starts paying.*
