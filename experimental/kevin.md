...
A combat chef. Raised in a temple — which one isn't decided; Drew suggested one of the Capital's (`places/capital/capital.md` — Sun, Moon, Rivers, or Forge), but Kevin's leaning toward something more isolated instead, and there isn't an existing isolated temple in canon that already fits, so this is genuinely open rather than a pick between known options.
...
- `places/capital/capital.md` — the Capital's own temples, for contrast with what Kevin's leaning away from
