...
**Self initiative-shift bug fixed (Drew) — a self shift must never grant a free action or a skip.** BEFORE: `_apply_shift`'s current-actor branch (`i == 0`) did `pending_turns.append(target)` on any positive amount — so if the current actor shifted ITSELF +1 it got a bonus turn. Latent, not live (no card self-shifts as the current actor today: Interrupt/Urgency's `initiative_shift(me,+3)` run in DEFENSE hooks where `me` is the defender at index >=1, not the actor), but wrong by Drew's rule: "giving yourself +1 should never give an extra action, and vice versa." WHY it was there: the branch conflated two cases — the current actor shifting itself vs. a card shifting the current attacker from a defense. NOW: the `i == 0` branch repositions instead of granting a turn — a positive self-shift makes you come around SOONER next cycle (removed from front, reinserted at `max(1, nr-amount)`, clamped to "next up" so it is never index 0 / never a bonus turn); a negative self-shift sends you to the back (never a skip, `skip_turns` untouched). Side effect (intended, consistent with earlier tempo-denial fixes): Delay's DEFENSE (`initiative_shift(foe,-3)` where foe = current attacker) was a silent no-op under the old branch and now actually pushes the attacker to the back. Non-current-combatant forward-cross bonus turn is preserved (regression-checked). Both tournaments unchanged (Steele 63.2, Team B 64.6). Invariant still holds: positive never slower, negative never faster — now including self-targets.
