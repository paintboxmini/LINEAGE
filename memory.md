...
**Initiative Shift fully implemented (Drew):** POSITIVE shift now grants the target an extra turn immediately after the current turn (fires right after the turn that caused the marker-crossing) — engine.pending_turns queue, drained in both run loops. NEGATIVE shift = target skips a turn; this also FIXED negative shift in teams (was `abs//n` = 0 skips for |amount|<team-size, so Mockery/Delay were silently doing nothing in a 6-person battle — now always 1 skip). Simplified to one crossing per shift. Effect: Interrupt/Delay tempo denial now bites in teams (Tempo beats Vanguard 99%). 1v1 unchanged (Sage 89, Steele 63.6). Battles terminate (~28 turns, no runaway). STILL a no-op: Predictable (Study def) — a peek-at-foe's-reveal that breaks blind simultaneity; needs a policy branch to consume the peek (offered to Drew, ~15 lines).
