...
**Why the dock reads it wrong — and what it gets right.** Nobody here knows any of that. What they see is a boatload of bodies gone wrong for no reason anyone can name, and the only word a port has for that is **cursed**. The word is wrong, and it is the word that will send anyone hunting an actual curse straight up the river to Pneum, where they will meet people who chose this, are proud of it, and will warmly explain that he has it backwards.

**But the fear underneath the wrong word is correct.** These are ordinary communicable diseases — real ones, spreading the way real ones do (`places/pneum.md`, What Is Actually Happening). Nothing aboard was ever touched by the Unheld; the Unheld changes what a body is and has never once made anyone ill (`world/the-unheld.md`). **So the Compact is not being superstitious. It is running a quarantine, correctly, for reasons it cannot articulate and would not believe if you told it.**

That is the hard shape of this whole thread: the port is right by accident, the congregation is sincere and lethal, and the party is the only group on the dock with no theory to protect.
