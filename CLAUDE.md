...
3. **Design Principles** (`agent-tools/design-principles.md`) — what makes content well-made: mechanics reflect the fantasy, ecology drives mechanics, a deck is a collection of behaviors, encounters teach through interaction. Violating one doesn't break anything the engine checks — it makes the content weaker.
4. **Exemplars** (`agent-tools/exemplars.md`) — concrete implementations chosen because they demonstrate Design Principles well. Extract the principle; don't copy the specifics. A creature can be a strong exemplar and still be nothing like the next one built from the same principles.

They blur because a real piece of content usually touches more than one at once — that's expected. The bug is a *file* absorbing another layer's job rather than staying narrow and pointing outward. This happened twice already, the same way: rule-definition bookkeeping quietly re-derived inside `invariants.md`, and then — even after that fix — design-craft principles ("a deck expresses behavior," "ecology drives mechanics") mislabeled as *invariant* right alongside genuinely computational facts like the turn-count-vs-wheel example, because both had been called by the same name. Both caught, both fixed. See `rules/invariants.md`'s header and `agent-tools/design-principles.md`'s header for the corrected, narrower scope of each.
...
- Ambiguity has levels — a different axis from Authority below (this measures confidence in a reading, not who may ship the result): **harmless** — wording, flavor, implementation detail: resolve silently; **design choice** — pick one and log it in `memory.md`; **constitutional** — formulas, keywords, cosmology, anything Authority 3 in the Canon Gate below: stop and surface. The cost of an unnecessary clarification is lower than the cost of a silently mutated design.
