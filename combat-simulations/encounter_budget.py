...
Caveat that governs every number below: roughly five in six Effect halves
are executed (see `effects.py`); the rest are narrated and therefore absent
from these runs. The tables were originally measured with none of them
running, on the argument that Effects push both sides up — and when they
started running, the thresholds did not move. Re-measured 2026-09-18 at
three thousand fights a row: every row inside about a point of the old
figure. Treat a 50% line as "a real fight" rather than a precise coin flip,
and expect drift from a party built around the cards still narrated.
"""
