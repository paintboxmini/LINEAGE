...
- **Exhaust has two contradictory zone rules in canon, and the Ashfall arc is built on the one the glossary rejects.** `rules/card-glossary.md` says Exhaust "goes directly into your hand when applied — not into the deck." Core cards obey this (OVERDRIVE, UNMAKE both add to hand). But six Ashfall-arc files put it in the *deck* instead: `cards/tithe-engine-ashfall.md` (which also prints its own local EXHAUST text saying it stays in hand — internally contradictory in one file), all three Exhaust cards in `cards/ashgrazer-ashfall.md`, CORRECTION LOAD in `cards/alignment-marshal-engine.md`, the Alignment Marshal's damage passive, and the Trisect's Fuel Seed action. This is not a typo cluster — it is a coherent designed subsystem (Tithe Engine seeds decks → Ashgrazers hunt the seeded Exhaust as prey-signal, stated outright in `bestiary/ashgrazer.md`). **Authority 3: either the glossary widens or six files of Ashfall content change, and both alter how existing content reads.** Surfaced 2026-08-02 while wiring the sim's Exhaust path. The sim currently models the glossary (hand only). *Trail: memory.md, the 2026-08-02 sim entry.*
