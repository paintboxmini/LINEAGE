...
- **Barbed Wrap, Carrion Feather, and Split Wedge are referenced across six files but have no `items/` entry.** They appear in `locations/roadhouse.md` (the barracks chest), `characters/aege.md` (the Feather is her parting gift to the party, a real handoff in Session 1 routing), `locations/vultures-nest.md`, `rules/items.md`, and `quests/washed-ashore.md` — but no file gives any of them mechanics. `memory.md` used to point at an `items/briarwoods-items.md` that has never existed; that phantom reference was removed in the 2026-08-02 Release audit, leaving the gap itself visible here instead. Surfaced by the Release reference sweep.
