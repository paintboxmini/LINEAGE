...
- **Two card-name collisions: COIL's CHARGE and TRAMPLE share names with different core cards.** `cards/minotaur-coil.md`'s versions are genuinely distinct designs (corridor-charge with bonus damage; size-splash with Thorns) from `cards/red-body.md`'s (mutual frontline move; bonus action on Collapse). Only the core versions are registered in the sim, so any deck list naming them resolves to the wrong card. Both COIL designs are worth keeping — the fix is renaming them, which is Drew's call. (A third collision, PULL, was resolved 2026-08-01: Stonecoil's copy was an identical stale duplicate, removed with a promotion note per the GORE precedent.) *Trail: memory.md, the bestiary deck build.*
