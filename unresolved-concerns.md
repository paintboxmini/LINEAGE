...
- **Senshi's Bone Collector commission may be the wrong denomination.** `locations/gilded-tusk.md` has him offering **35 silver** (3.5 gold) for a fresh Bone Collector specimen delivered intact. His published rates in `items/turnroot-weald-items.md` are *"~40 gold (Senshi's going rate)"* and *"~35 gold (Senshi's going rate)"* — for an Emerald Frog and a Bicolor Spider, both trivially safer to obtain than a CTR 10 boss. Same buyer, same kind of transaction, same numeral, denomination 10× apart, and the dangerous job pays least. Either it is a unit slip, or Senshi is deliberately lowballing a job he knows is dangerous, which would be characterful and should then be said out loud. Not changed unilaterally because a price is canon. *Surfaced by the 2026-08-02 gold-pacing currency sweep.*
- **Twelve of the fourteen files in `quests/` still have no reward line.** Only `hollow-below-briarwatch.md` (built as the worked example, 2026-08-02) and `the-man-who-buys-wounds.md` (Weck pays coins by design) address payment at all. The pacing rule now exists (`rules/equipment.md`, Pacing) so the number is derivable per quest; this is applying it, best done as each quest is next touched rather than in one sweep.
## Canon & World
