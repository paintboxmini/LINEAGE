...
- **`rules/core-rules.md`'s Card Anatomy die table is stale — reads d6/d4/d2, the pre-2026-07-22 scale.** The real scale is d8 raw power / d6 utility / d4 precision (`rules/cards.md`, `rules/player-guide.md`, both correct). Not a one-line fix so much as a scoping question: now that `player-guide.md` exists and covers the same material accurately, is `core-rules.md` still a live player-facing quick reference worth maintaining in parallel, or has the guide superseded it? Answer that before syncing the table, or it'll just drift again. *Trail: memory.md, the 2026-08-01 player-guide build and the Item Action rename.*
