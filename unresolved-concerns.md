...
- **Two unrelated creatures both have a card literally named FREEZE** — Briarwatch Jackalope's (now registered in the sim) and Flapjack Octopus's (still unregistered, different text). Not an active conflict today since only one is wired in, but the sim's card registry is name-keyed — only one "FREEZE" can ever exist in it — and it's a real canon naming collision, not just a sim inconvenience. Needs a rename or other resolution whenever Flapjack's gets registered. *Trail: `cards/briarwatch-jackalope.md`, `cards/flapjack-octopus.md`; memory.md, the 2026-07-28 5:59 PM entry.*
