...
- Initiative: **Wait** = forfeit action, choose a later slot (the party's sequencing tool, unchanged). The wheel and Initiative Shift's definitions were rewritten this session (sliding, join/leave, eight edge cases ruled) — check `rules/card-glossary.md` and `rules/combat.md` directly for the full mechanic; this summary is due a real rewrite at next Sync.
