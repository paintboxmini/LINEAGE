...
- Initiative: **Wait** = forfeit action, choose a later seat (the party's sequencing tool, unchanged). The wheel and Initiative Shift's own definitions are being rewritten fresh — check `rules/card-glossary.md` and `rules/combat.md` directly rather than relying on this summary until that's done.
