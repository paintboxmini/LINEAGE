...
- **Standard fights, where the players hold a real advantage, should resolve in about 3 turns per combatant.** Validated against the live 2v2 test: a deliberately fair matchup with no player-side edge ran 5–6 turns per combatant instead — not a miss, exactly what a fight with no player advantage should look like, the same relationship a boss fight is supposed to have with a standard encounter. Measure future tests against both numbers — 3/combatant signals the players had a real edge, 5-6+/combatant signals the fight reads as boss-tier rather than standard.
- **Red's combat power is inherent — don't fight it. Balance the colors through card effects instead:** green and blue pay for less raw damage with stronger effects.
- **Initiative Shift's impact peaks at 4 combatants — not a floor, a peak.** 2 (a duel) is degenerate: no interior to reposition within, so every shift collapses to no-op, bonus turn, or skip. 3 is reduced from peak. 4 is where it does its best work. Nothing established about what happens past 4.

*(The pillar-risk principle and the telegraphed-over-hidden rule moved to `rules/combat.md`, Core Combat Philosophy, 2026-08-11 — Drew's correction: these describe how the game itself works, not how to build well-made content for it, so this file was the wrong home. Full adoption trails for both stay archived at `archives/key-design-decisions.md`.)*
