...
**Action: Play a Card.** Frost attacks Scratcher A with **PROFILE** (Blue, Mind + d6, Range: Both). Range check: Frost Frontline → A Frontline is melee range; PROFILE says Both, so it's legal from anywhere.
...
**Action:** Steele attacks Scratcher A with **STRIKE** (Red, Body + d10, Melee — both Frontline, legal).
...
**Action:** B attacks Frost with **SKITTER AWAY** (Blue, Mind + d4, Ranged: Both). Range check: Backline → Frontline is not melee range, so Both is legal.

Frost holds REFRACT and STRIKE, and wants both for his own turn. Max incoming damage here is Mind 1 + d4 = 5. **Frost declines to defend.** Per resolution step 3: *no defense → attacker wins automatically.* Damage: 1 + d4 *(rolls 2)* = **3**. Frost 12 → 9. SKITTER AWAY Effect: *after attack, may reposition* — B declines; Backline is already exactly where it wants to be. Unlike a discard effect, this doesn't touch Frost's hand.
...
End of turn: Frost's hand: REFRACT, STILLNESS. A's hand: empty. A's deck: SHADE AWAY.
...
**Action:** A attacks Frost with **SHADE AWAY** (Green, Soul + d4, Melee — both Frontline, legal).

Frost's hand is REFRACT and STILLNESS — both **Ranged**, and both combatants are Frontline, so melee range applies and neither can legally defend here (`rules/card-glossary.md`, Range; the same rule that gates attacking gates defending too). **Frost holds two cards and still has no legal defense** — functionally an empty hand. Per resolution step 3, A wins automatically.

Damage: Soul 2 + d4 *(rolls 2)* = 4. SHADE AWAY Effect: A gains **Evade**. Frost 9 → **5**.

Because there was no legal defense to choose, nothing left Frost's hand — REFRACT and STILLNESS are both still there. A's hand after playing SHADE AWAY: RAKING CUT. A's deck: SKITTER AWAY, NIP AND TEAR.

### Steele's turn

Draws to 3 → draws BRACE. Hand: DEAD HEAT, CALCULATE, BRACE.

**Action:** Steele attacks Scratcher B with **DEAD HEAT** (Red, Body + d8, Ranged). Range check: Frontline → Backline is not melee range — Ranged is legal.
...
Damage: Body 4 + d8 *(rolls 5)* = **9**. B: 11 → 2. DEAD HEAT's Effect only fires on a tie, and this isn't one, so it does nothing here — there was no Defensive Bonus in play to cancel anyway.
...
Frost's hand is REFRACT and STILLNESS — neither was ever legally playable this fight (both Ranged, and every exchange so far has been melee range), so both are still sitting there. Draws to 3 → draws PUSH from his deck. Hand: REFRACT, STILLNESS, PUSH.

**Action:** Frost attacks B — both Frontline now — with **PUSH** (Red, Body + d6, Melee). B reveals **NIP AND TEAR** (Green, Melee — legal, both Frontline now). **Green beats Red — defender wins.** No damage. NIP AND TEAR Defensive Bonus: *Heal 1.* B: 2 → 3.
