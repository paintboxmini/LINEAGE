...
**Treat this ladder as provisional.** Drew's read as of 2026-09-17 is that it will break before long, and the reason is visible above: it is described as a game-wide rule and it already carries two named exceptions out of the handful of cards that use it. A rule that has to be argued around every time it is applied is on its way to not being a rule. Don't rest a decision on it alone. RETORT's seat in the Oracle expansion was argued partly from this ladder — that is the first thing to revisit if the ladder goes.
...
Twenty-one cards were written on 2026-09-08 as an expansion, before the bench-first rule above existed, and they are kept because they are good cards, not because the deck needed them. Eighteen of them still sit in the core lists as bench rather than in either Oracle set — CLEAVE, MAUL, SKEWER, SHOULDER, EXPEND, ANCHOR and HAMMER in Red; DECODE, REDIRECT, UNRAVEL, CORNER, PROBE, DISSECT and INVERT in Blue; HARMONIZE, STIR, SHELTER and ENTREAT in Green. The other three have since been drafted out of it: PROVOKE moved Green → Red on 2026-09-09 and took a Red slot in the Oracle expansion, and CHANNEL and CONFRONT joined that set on 2026-09-17. Four of the twenty-one carry shapes the pool had never held: INVERT cancels the other side's Effect for an exchange, CORNER locks both combatants in place rather than charging one with Rooted, CHANNEL is the only modal card — pick one of three, on its attack half — and CONFRONT is the pool's only Green Counter Attack. The rest are available for creature decks, character decks, and any later Oracle seat.
