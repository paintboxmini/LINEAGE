...
Twenty-one cards were written on 2026-09-08 as an expansion, before the bench-first rule above existed, and they are kept because they are good cards, not because the deck needed them. Seventeen of them still sit in the core lists as bench rather than in either Oracle set — CLEAVE, MAUL, SKEWER, SHOULDER, EXPEND, ANCHOR and HAMMER in Red; DECODE, REDIRECT, CORNER, PROBE, DISSECT and INVERT in Blue; HARMONIZE, STIR, SHELTER and ENTREAT in Green. The other four have since been drafted out of it: PROVOKE moved Green → Red on 2026-09-09 and took a Red slot in the Oracle expansion, and WHAT DO YOU NEED, CONFRONT and SOLD THE FEINT joined that set on 2026-09-17 (the first and last were written as CHANNEL and UNRAVEL; see the naming note below). Four of the twenty-one carry shapes the pool had never held: INVERT cancels the other side's Effect for an exchange, CORNER locks both combatants in place rather than charging one with Rooted, WHAT DO YOU NEED is the only modal card — pick one of three, on its attack half — and CONFRONT is the pool's only Green Counter Attack. The rest are available for creature decks, character decks, and any later Oracle seat.
...
**Worked example, 2026-09-17.** Five cards seated in the Oracle expansion were renamed for breadth alone — UNRAVEL, UNNAME, CHANNEL, PARADOX and ALIGN became SOLD THE FEINT, NO REPLY, WHAT DO YOU NEED, BAD BEAT and MATCHED PAIR. Not one Effect line changed. Every one of the old names was a wide abstract verb a player could spend on almost any noncombat attempt, which is a second card's worth of power handed out in a starting deck; every new one is a concrete, narrow thing, in the register the pool already uses for OPEN GUARD, CALLED SHOT, TABLE STAKES and STARING CONTEST. A first deck should teach the combat system, not hand out skeleton keys.

ALIGN was the one worth freeing rather than retiring. Its effect — Scry 2, check whether the two match — was never about alignment, so the word was doing no work where it sat. It now names a Green bench card about allies lined up two ways at once, by position and by the initiative order. A broad name is fine there: the bench is where later cards come from, and a card that earns the word can afford it.
