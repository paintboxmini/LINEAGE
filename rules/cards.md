...
*That premium was set when Body was a ×3 multiplier on HP rather than ×4, so it is fair to ask whether the damage gap should have closed when the HP weight went up. **It was measured on 2026-09-19, the answer came back no, and that answer did not survive a second benchmark.** In duels the three colours finish within 1.4 points of each other; run the same three archetypes as a party against a real encounter and they finish 73.7 points apart, with the Soul-primary one at 24.3%. Both numbers are from the same build. What separates them is how often a Passive gets played, which the duel and the group fight disagree about completely — so the ×3 → ×4 question is **open**, not settled, and nothing here should be changed on the strength of the duel figure alone. `combat-simulations/README.md`, What Passives are actually for.*
