...
Multiple shifts applied to the same token at once sum into one net shift before it applies. **A combatant shifting itself on its own turn is the exception, and it is not a small one.** They are standing on the marker's slot, so the distance to it is zero and every positive shift would read as a crossing — QUICKEN would be a free extra turn, every time. It is not: nothing carries you past a point you are standing on. Measure it the way this section says to, against when their own next turn would have arrived. Having just acted, that is after everyone else; a shift of X makes it X sooner than that, down to a floor of acting next. No chip, no skip. The mirror case is simpler to state and harder to live with. A negative shift is later, and a combatant who has just acted is already last, so there is no later slot for the wheel to give them. **This applies to the attacker whoever is doing the shifting** — a defender pushing the attacker back is pushing against the end of the ring, and it places no chip on anyone either way.

*Open, and it is a design question rather than a wheel problem: that leaves the defence halves of DELAY, DOUBLE DOWN, HASTEN, INTERRUPT, RETALIATE and STEAL with nothing to do, since all six shift the attacker during the attacker's own turn. Their attack-half counterparts — DELAY, DISTRACT, MOCKERY, TURN — aim at someone who is not acting and work normally. Making "later than last" mean anything requires the wheel to express a skipped lap, and how many laps a -2 is worth is a call for whoever prices the cards.*
