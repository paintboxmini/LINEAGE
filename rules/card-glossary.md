...
<!-- print:skip-start -->
*The number before each keyword is how many **core** cards use it — the four lists a deck actually builds from (`cards/red-body.md`, `cards/blue-mind.md`, `cards/green-soul.md`, `cards/colorless.md`), **counted 2026-09-09 and a snapshot of that day** — `printing/manifest.txt` holds the current size of those lists. Creature signature cards are deliberately excluded: they are one-offs written for a single stat block, so counting them tells you nothing about how common a keyword is in play. These counts are stripped from the printed packet (`printing/generate-rules-pdf.py`), so they exist for maintenance here and nowhere else.*

*A keyword counts when a card names it in an Attack, Effect, Defense Effect, or Special Rule line, once per card however many times it appears. Conditional references ("if the defender is Rooted") and negations ("cannot be Evaded, Resisted, or affected by Blind" — CERTAIN STRIKE, the only live example) don't count toward the keyword being referenced; those cards are counted under whatever they actually grant instead. CERTAIN STRIKE is the sharp case: since 2026-09-17 its defence half grants Ward rather than Resist, so it counts under Ward and nothing else. Resist, Evade and Blind all appear on the card and none of them count. Case and inflection don't matter — several cards write "unpreventable" in lower case mid-sentence, STEAL says "Exiled" rather than "Exile", and three cards write "Positive Status Effect" singular. Match the word stem, not the glossary header. Debuff sits at 0 on purpose: no card names it, and it exists for Ward's definition to point at.*
...
Initiative Shift always moves the token the full requested distance. Where a shift would run past the end of what the wheel can express, place a chip instead of changing the movement.
...
Multiple shifts applied to the same token at once sum into one net shift before it applies. **A combatant shifting itself on its own turn is the exception, and it is not a small one.** They are standing on the marker's slot, so the distance to it is zero and every positive shift would read as a crossing — QUICKEN would be a free extra turn, every time. It is not: nothing carries you past a point you are standing on. Measure it the way this section says to, against when their own next turn would have arrived. Having just acted, that is after everyone else; a shift of X makes it X sooner than that, down to a floor of acting next. No chip, no skip. The mirror case is the one that reads backwards if you are not careful. A combatant on the marker's slot is at position 0, but their **next** turn is a full table away — they have just acted, so everyone else goes before they come round again. Read their slot as their place in the queue and every shift on them comes out inverted.

So measure from where their next turn actually was. **A negative shift of X puts X more turns in front of theirs**, whoever cast it. At a table of four, where a turn normally comes round after three others: -1 is after four, -2 after five, -3 after six. The ring says it in two parts — the token sits X slots along and is passed over once when the marker first reaches it — but what a player needs to know is the first sentence. One more person goes before you for each point.

**It is a later turn, not a lost one**, which is what keeps this clear of the Oracle content rule's ban on costing an enemy a whole attack (`rules/cards.md`). The turn arrives, and the wheel shows exactly when.

At the table this is two motions and they are easy to do by hand: slide the token along, and pass it over the first time the marker reaches it. A big delay at a small table passes it over more than once — at a table of three, a -3 is skipped the first time it comes round and acts again after five others. Every delay is reachable at every table size; there is no case the ring cannot say.

For everyone else, the crossing rule stands. If a positive shift's distance is **greater than the distance to the marker's own slot** — so the token would be carried past the point where it must act now — the target instead receives an immediate extra turn, taken as soon as the currently-resolving turn finishes. The combatant already acting when this happens is not shorted a turn, but doesn't get a second one either: the slide moves them to a new slot, and the marker skips that slot when it reaches it, since they already acted this lap. That skip is specifically compensation for the bonus turn just granted — an ordinary bystander displaced by sliding, with no bonus turn triggering it, simply acts normally when the marker reaches wherever it landed. A negative shift is the mirror case: if its math would let the target act sooner than the marker's normal progression allows, the shift still moves the target's token and slides the wheel in full, but the target's turn is skipped the first time the marker reaches its new slot — it acts normally starting the next lap. Reshifting a token that already carries a pending skip or bonus chip removes the pending chip — the token then resolves normally under the new shift, whatever slot it lands on.
