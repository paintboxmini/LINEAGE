...
*The number before each keyword is how many cards in `cards/` use it — a snapshot as of 2026-08-05, not a live count. It'll drift as cards are added or reworked; recount rather than trust it once it's been a while. Recounted across all 347 card blocks in `cards/*.md` — 344 colored plus the 3 colorless (up from 240 at the 2026-07-28 snapshot, which predates the bestiary signature-card work; block total bumped 2026-08-06 for Wrackclaw and Hullback's six new cards, 2026-08-07 for Fermata's three, 2026-08-08 for the Flower Snake's six, the same day for the Kiwi Bird's four, and 2026-08-10 for STILL POINT; **down one 2026-08-17** when the Unheld card file and its sole card LOOSE GRIP were deleted, which also drops Initiative Shift X by one; **up two 2026-08-17** for Rasp's poison rework, which nets +2 blocks and touches Weak, Vulnerable, and Evade; **up six 2026-08-17** when a prose scan found six cards that had been living inside `characters/kaine/` and `characters/mirel/` instead of `cards/` — Kaine's four and Mirel's two, touching Staggered, Armour, Deadly, Exile, Weak, Evade and Rooted). Counted mechanically: a keyword named in an Effect, Defensive Bonus, Special Rule, or Attack line, excluding conditional references ("if the defender is Rooted") and negations ("ignores Evade"). Per-keyword numbers still carry the 2026-08-05 snapshot's drift (see `unresolved-concerns.md`) — this update only kept the block total accurate, same discipline as the last time it drifted, plus the keywords each new batch of cards demonstrably touches (Fermata: Locked, Sealed, Vulnerable, Resist; Flower Snake: Evade, Weak, Vulnerable, Resist, Deadly, Rooted, Ward; Kiwi Bird: Evade, Resist; STILL POINT: Anchored, Scry, Ward).*
