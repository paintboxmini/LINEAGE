...
Multiple shifts applied to the same token at once sum into one net shift before it applies. **A combatant shifting itself on its own turn is the exception, and it is not a small one.** They are standing on the marker's slot, so the distance to it is zero and every positive shift would read as a crossing — QUICKEN would be a free extra turn, every time. It is not: nothing carries you past a point you are standing on. Measure it the way this section says to, against when their own next turn would have arrived. Having just acted, that is after everyone else; a shift of X makes it X sooner than that, down to a floor of acting next. No chip, no skip. The mirror case is where the wheel runs out of ring. A negative shift is later, and a combatant who has just acted is already last, so there is no later slot to move them to. **The delay is spent as skipped laps instead** — the token stays where it is and misses that many of its own turns, one chip per lap, cleared one at a time as the marker reaches them. "Where a shift would run past the end of what the wheel can express, place a chip instead of changing the movement," counted. This applies to the attacker whoever is doing the shifting: a defender pushing the attacker back is pushing against the end of the ring, and it costs the attacker turns rather than position. No bonus turn is earned and no third party is skipped in compensation — that machinery belongs to a shift that crosses the marker, and standing on it is not crossing it.

> **This collides with the Oracle content rule and the collision is not small.** `rules/cards.md` bars a starting-deck card from costing an enemy a whole attack or a whole defence "however it is worded", and says to ask what the target loses rather than which word the card used. What a target now loses to a defensive negative shift is whole turns: **RETALIATE and DOUBLE DOWN cost one, INTERRUPT costs two**, and all three are seated. DELAY and STEAL do the same from the bench. Either those cards want re-pricing or the ban wants an exception for initiative; that is a decision about the cards, not about the wheel, and it has not been made.
