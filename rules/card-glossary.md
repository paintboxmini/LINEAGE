...
**It is a later turn, not a lost one**, which is what keeps this clear of the Oracle content rule's ban on costing an enemy a whole attack (`rules/cards.md`). The turn arrives, and the wheel shows exactly when. A delay that comes out to an exact multiple of the table size is the one thing the ring cannot say, because the slot it needs is the marker's own; it lands one short rather than one long.
