...
- **The countdown vector is the canonical state; the wheel is its human rendering.** The engine reasons in turns-until-next-action; players reason in seats. Same state, two coordinate systems. A shift is a single relocation in that vector: the target's next turn moves exactly X and the new seat persists; each of the X turn-slots it passes over nudges one turn the opposite way (those slots' owners may include someone's next-lap turn); everyone else's schedule and relative order are untouched. The table's insert-and-slide gesture computes exactly this — the visualization and the math are the same operation.
