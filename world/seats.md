...
**Aurora is fused to a physical Loom, and it makes this Seat the first real case of full convergence** (`world/archons.md`, Multiple Bearers, One Seat). The Loom is the literal machinery she is chained to, deep beneath the city — her hair splayed out in every direction, feeding directly into it as the thread the Weavers' wider Grid eventually draws from (`factions/the-weavers.md`). Built with intent, permanently fused to a living Archon, run without pause for a century: the Loom has reached its own Embodiment. The capital itself — that same binding load-bearing through every district, foundation, and utility line the city has, for just as long — has reached its own Hallowing. Aurora, the Loom, and Eclipseria are Incarnate, Embodied, and Hallowed on the same Seat, at the same time: the first Seat in the setting to actually complete the convergence its own name always implied.
